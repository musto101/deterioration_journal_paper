...
ctrl <- trainControl(method = 'cv', number = 5, classProbs = T, 
                     ...
                     verboseIter = F, allowParallel = TRUE)

svmRadialGrid <- expand.grid(C = seq(0, 1, .1),
                             sigma = seq(0, 1, .1))
...
for (j in 1:mcRep) {
  ...
  for (n in 1:nrfolds){
    ...
    # tuning
    svmRadialModel<-train(last_DX ~ ., training, method = "svmRadial", 
                          metric = "ROC",
    ...
    evalResults$rf <- predict(svmRadialModel, test, type = "prob")[, 1]
    evalResults$newPrediction <- predict(svmRadialModel, test)
...
post <- Sys.time()
# write.csv(mcPerf, 'data/mci_svmMcPerf.csv')
# write.csv(totalprobabilities, 'data/mci_svm_probabilities.csv')
